...
void UPDATE_nrMon(CHARACTER x){//test to update number of monsters user has killed.
	sqlite::sqlite db(scores);//Open database***
	
	auto cur = db.get_statement();
	try {
	cur->set_sql("SELECT count(*) FROM tbl_monsterkills where username=?;");
	cur->prepare();
	cur->bind(1,x.get_name());
	cur->step();
	if (cur->get_int(0) == 1) {
		cur->reset();
		cur->set_sql("UPDATE tbl_monsterkills SET nrEnemies=? where username=?");
		cur->prepare();
		cur->bind(1,nrEnemies);
		cur->bind(2,x.get_name());
		cur->step();
	} else {

		cur->reset();//CUR RESETS AFTER EACH QUERY.
		cur->set_sql("INSERT INTO tbl_monsterkills (username,nrEnemies)"
			"VALUES (?,?);");//creation of new entry attempt

		cur->prepare();

		cur->bind(1,x.get_name());
		cur->bind(2,nrEnemies);
